...
pub mod testnet_v2 {
	...
	fn read_points(path: &str) -> std::io::Result<(Vec<G1>, Vec<G2>)> {
		let bytes = include_bytes!("extracted_1024.data");
		...
		let a: Vec<G1> = g1s
			.chunks_exact(48)
			.map(|b| G1::deserialize_compressed(b).unwrap())
			.collect();
		let b: Vec<G2> = g2s
			.chunks_exact(96)
			.map(|b| G2::deserialize_compressed(b).unwrap())
			.collect();
	...
	mod tests {
		...
		fn test_consistent_testnet_params() {
			...
			let pmp2_points = read_points("kate/src/extracted_1024.data").unwrap();
			let pmp2 = m1_blst::M1NoPrecomp::new_from_powers(pmp2_points.0, pmp2_points.1);

			let evals = DensePolynomial::<Fr>::rand(1023, &mut thread_rng()).coeffs;
			let domain = GeneralEvaluationDomain::<Fr>::new(1024).unwrap();
			let poly = domain.ifft(&evals);

			let cm1 = pmp.commit(&poly).unwrap();
			let cm2 = pmp2.commit(&poly).unwrap();
			assert_eq!(cm1.0, cm2.0);

			let point = gridgen::domain_points(4).unwrap()[1];
			let witness = pmp.compute_witness_polynomial(poly, point).unwrap();
			let proof = pmp.open(witness).unwrap();

			assert!(pmp.verify(&cm1, point, evals[1], &proof).unwrap());
		}
	}
}
